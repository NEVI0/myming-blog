...
import { POST_VALIDATION, PostAbstract } from '@domain/entities';
...
export default function Form({ post }: FormProps) {
  ...
  const [note, setNote] = useState<string | null>(post.note ?? null);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success(
        'Post atualizado com sucesso! Você será redirecionado para a página do post...'
      );

      redirectPath = `/post/${post.id}`;
    } catch (error: any) {
      toast.error(
        handleErrorMessage(error, 'Erro ao atualizar o post. Tente novamente!')
      );
    } finally {
      setIsLoading(false);
    ...
    <form className="flex flex-col gap-16" onSubmit={handleSubmit}>
      ...
      <section className="flex flex-col md:flex-row gap-8 md:items-center justify-between">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          >
            {isLoading ? 'Editando...' : 'Editar post'}
          </Button>
        </div>
      </section>
    </form>
